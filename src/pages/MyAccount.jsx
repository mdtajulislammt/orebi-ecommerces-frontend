...
import { useGetMeQuery, useGetMyOrdersQuery, useUpdateUserMutation } from "../features/api/apiSlice";
...
const MyAccount = () => {
  ...
  const { data: userData, isLoading: userLoading } = useGetMeQuery(undefined, { skip: !token });
              ...
              <div className="p-4 lg:p-6 border-b border-gray-50 bg-gray-50/30 hidden lg:block">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 lg:w-12 lg:h-12 rounded-xl lg:rounded-2xl bg-black text-white flex items-center justify-center font-black text-lg lg:text-xl shadow-lg shadow-black/20">
                    <img src={userData?.data?.avatar_url} alt="" className="w-10 h-10 lg:w-12 lg:h-12 rounded-xl lg:rounded-full" />
                  </div>
                  <div>
                    <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest leading-none mb-1">Customers</p>
                    <p className="text-xs font-bold text-primary-color truncate max-w-[120px]">{userData?.data?.name}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="overflow-x-auto no-scrollbar">
                <List className="flex lg:flex-col p-2 min-w-max lg:min-w-0">
                  {[
                    ...
                    { id: "accountDetails", label: "Account Details", icon: <FaUserEdit /> },
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6 mt-6 lg:mt-10">
                    <div className="group p-6 lg:p-8 bg-white rounded-[1.5rem] lg:rounded-[2rem] border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-500">
                      <div className="flex justify-between items-start mb-4 lg:mb-6">
                        ...
                        <span className="text-[8px] lg:text-[10px] font-black bg-blue-50 text-blue-600 px-2 lg:px-3 py-1 rounded-full uppercase tracking-tighter">Updated</span>
                      </div>
                      <p className="text-gray-400 font-bold text-[10px] uppercase tracking-wider mb-1">Total Purchases</p>
                      <h4 className="text-2xl lg:text-3xl font-black text-primary-color">{orders.length}</h4>
                    </div>
                    ...
                    <div className="group p-6 lg:p-8 bg-white rounded-[1.5rem] lg:rounded-[2rem] border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-500">
                      <div className="flex justify-between items-start mb-4 lg:mb-6">
                        ...
                        <span className="text-[7px] lg:text-[8px] font-black bg-green-50 text-green-600 px-2 lg:px-3 py-1 rounded-full uppercase tracking-tighter">Premium</span>
                      </div>
                      <p className="text-gray-400 font-bold text-[10px] uppercase tracking-wider mb-1">Status</p>
                      <h4 className="text-2xl lg:text-3xl font-black text-primary-color">Excellent</h4>
                    </div>
                  </div>
              </div>
            )}
              ...
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <div className="flex items-center justify-between mb-6 lg:mb-10">
                    <div>
                      ...
                      />
                      <p className="text-gray-400 text-xs lg:text-sm mt-1">Manage your order history</p>
                    </div>
                  ...
                  {ordersLoading ? (
                    ...
                    <>
                      ...
                      <div className="hidden md:block overflow-hidden rounded-[2rem] border border-gray-50 shadow-sm">
                        <table className="w-full text-left">
                          <thead className="bg-gray-50/50">
                            <tr>
                              <th className="px-8 py-5 font-black text-[10px] uppercase tracking-widest text-gray-400">Reference</th>
                              <th className="px-8 py-5 font-black text-[10px] uppercase tracking-widest text-gray-400">Order Date</th>
                              <th className="px-8 py-5 font-black text-[10px] uppercase tracking-widest text-gray-400 text-center">Status</th>
                              <th className="px-8 py-5 font-black text-[10px] uppercase tracking-widest text-gray-400 text-right">Total Amount</th>
                            </tr>
                          </thead>
                            ...
                            {orders.map((order) => (
                              <tr key={order.id} className="hover:bg-gray-50/50 transition-colors group">
                                <td className="px-8 py-6">
                                  <span className="font-black text-sm text-primary-color">#{order.id?.slice(-8).toUpperCase()}</span>
                                </td>
                                <td className="px-8 py-6">
                                  <div className="flex flex-col">
                                    <span className="text-sm font-bold text-primary-color">{new Date(order.createdAt).toLocaleDateString(undefined, { day: 'numeric', month: 'short', year: 'numeric' })}</span>
                                    <span className="text-[10px] text-gray-400 font-bold uppercase tracking-tighter">{new Date(order.createdAt).toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })}</span>
                                  </div>
                                </td>
                                <td className="px-8 py-6 text-center">
                                  <span className={`inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${
                                    ...
                                    order.status === 'completed' ? 'bg-emerald-50 text-emerald-600' : 'bg-gray-100 text-gray-600'
                                  }`}>
                                    {order.status === 'pending' ? <FaClock className="text-[8px]" /> : <FaCheckCircle className="text-[8px]" />}
                                    {order.status}
                                  </span>
                                </td>
                                <td className="px-8 py-6 text-right">
                                  <span className="text-base font-black text-primary-color">${Number(order.total_amount).toFixed(2)}</span>
                                </td>
                              </tr>
                            ))}
                          </tbody>
                      ...
                      <div className="md:hidden space-y-4">
                        {orders.map((order) => (
                          <div key={order.id} className="p-5 rounded-2xl border border-gray-100 bg-gray-50/30">
                            ...
                            <div className="flex justify-between items-end">
                              <div>
                                <p className="text-xs font-bold text-primary-color">{new Date(order.createdAt).toLocaleDateString()}</p>
                                <p className="text-[10px] text-gray-400 font-bold uppercase">{new Date(order.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                              </div>
                              <p className="text-lg font-black text-primary-color">${Number(order.total_amount).toFixed(2)}</p>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="text-center py-16 lg:py-24 bg-gray-50/50 rounded-[2rem] lg:rounded-[2.5rem] border-2 border-dashed border-gray-100">
                      ...
                      <p className="text-gray-500 font-bold text-base lg:text-lg mb-2">No orders found</p>
                      <button 
                  ...
                  <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 lg:mb-12">
                    <div>
                      <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-black/[0.03] border border-black/[0.05] mb-4">
                        ...
                        <span className="text-[8px] lg:text-[10px] font-black uppercase tracking-[0.2em] text-black/50">Personal Space</span>
                      </div>
                      <Heading
                        tagname="h3"
                      ...
                      />
                      <p className="text-gray-400 text-sm lg:text-lg mt-2 font-medium">Manage your digital identity</p>
                    </div>
                  </div>
                  
                  {isEditing ? (
                    <form onSubmit={handleUpdateProfile} className="space-y-8 lg:space-y-10">
                        ...
                        <div className="flex flex-col items-center gap-4 mb-6 lg:mb-10">
                          <div className="w-28 h-28 lg:w-40 lg:h-40 rounded-[2rem] lg:rounded-[3rem] overflow-hidden bg-gray-50 border-4 lg:border-8 border-white shadow-xl relative group">
                            ...
                            <label className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 cursor-pointer transition-all duration-500 backdrop-blur-sm">
                              ...
                              <span className="text-white text-[8px] lg:text-[10px] font-black uppercase">Update</span>
                              <input type="file" className="hidden" accept="image/*" onChange={handleImageChange} />
                            </label>
                          </div>
                          <div className="px-3 py-1 rounded-full bg-gray-100 text-[8px] lg:text-[10px] font-black text-gray-400 uppercase">Identity Image</div>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-8">
                          <div className="space-y-2 lg:space-y-3">
                            <label className="text-[10px] lg:text-xs font-black text-gray-400 uppercase tracking-widest ml-1 block">Full Name</label>
                          ...
                          <div className="space-y-2 lg:space-y-3">
                            <label className="text-[10px] lg:text-xs font-black text-gray-400 uppercase tracking-widest ml-1 block">Contact Number</label>
                            <div className="relative group">
                              <span className="absolute left-5 lg:left-6 top-1/2 -translate-y-1/2 text-gray-300">
                                <FaPhoneAlt />
                              </span>
                    ...
                    <div className="space-y-8 lg:space-y-12">
                      ...
                      <div className="group relative p-1 rounded-[2rem] lg:rounded-[3rem] bg-gradient-to-br from-gray-100 via-white to-gray-50 shadow-xl shadow-gray-200/40 overflow-hidden">
                        <div className="relative z-10 bg-white rounded-[1.8rem] lg:rounded-[2.8rem] p-6 lg:p-10 flex flex-col lg:flex-row items-center gap-6 lg:gap-10 text-center lg:text-left">
                          ...
                          <div className="flex-1">
                            <p className="text-[8px] lg:text-[10px] font-black text-gray-300 uppercase tracking-[0.3em] mb-1 lg:mb-2">Primary Identity</p>
                            <h4 className="text-2xl lg:text-4xl font-black text-primary-color mb-2 lg:mb-3">{userInfo?.name || "Member Name"}</h4>
                            <div className="inline-flex items-center gap-2 px-3 lg:px-4 py-1 rounded-lg lg:rounded-xl bg-emerald-50 border border-emerald-100">
                              ...
                              <span className="text-[8px] lg:text-[10px] font-black text-emerald-600 uppercase tracking-widest">Verified Account</span>
                            </div>
                          </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-8">
                        <div className="p-6 lg:p-10 bg-white rounded-[1.5rem] lg:rounded-[2.5rem] border border-gray-100 shadow-sm">
                          ...
                          <p className="text-[8px] lg:text-[10px] font-black text-gray-300 uppercase tracking-[0.2em] mb-1 lg:mb-2">Email Address</p>
                          <p className="text-sm lg:text-lg font-bold text-primary-color break-all tracking-tight">{userInfo?.email}</p>
                        </div>
                        ...
                        <div className="p-6 lg:p-10 bg-white rounded-[1.5rem] lg:rounded-[2.5rem] border border-gray-100 shadow-sm">
                          ...
                          <p className="text-[8px] lg:text-[10px] font-black text-gray-300 uppercase tracking-[0.2em] mb-1 lg:mb-2">Phone Number</p>
                          <p className="text-sm lg:text-lg font-bold text-primary-color tracking-tight">{userInfo?.phone_number || "Not provided"}</p>
                        </div>
                      </div>
